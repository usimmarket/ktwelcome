...
function unifyByGroups(obj) {
  ...
  for (const g of KEY_GROUPS) {
    ...
    if (val !== undefined) g.forEach(k => (out[k] = String(val)));
  }
  return out;
}
...
exports.handler = async (event) => {
  try {
    const isForm = (event.headers && (event.headers['content-type'] || '').includes('application/x-www-form-urlencoded'));
    ...
    if (qsMode === 'debug' || values.mode === 'debug') {
      return {
        statusCode: 200,
    ...
    // 출력
    const modeHint = String(qsMode || values.mode || '').toLowerCase();
    const disp = (modeHint === 'print' || modeHint === 'inline') ? 'inline' : 'attachment';
